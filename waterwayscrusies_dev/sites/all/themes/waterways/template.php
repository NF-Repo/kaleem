 ...
 */
function waterways_theme($existing, $type, $theme, $path) {
  return array(
    'navbar_brand' => array(
      'variables' => array(
        'name' => NULL,
 ...
 */
function waterways_css_alter(&$css) {
 ...
 */
function waterways_process_html(&$vars) {
  if ($body_top = _waterways_region_blocks_markup('body_top')) {
    $vars['page_top'] = '<div class="conceal">' . $body_top . '</div>' . $vars['page_top'];
  }
  if ($body_bottom = _waterways_region_blocks_markup('body_bottom')) {
 ...
 */
function waterways_preprocess_page(&$vars) {
  $page = $vars['page'];
  ...
  _waterways_verify_jquery_update();

  // Connect Twitter Bootstrap library:
  if ($bootstrap_path = _waterways_detect_twitter_bootstrap()) {
 ...
 */
function waterways_process_page(&$vars) {
  $page = $vars['page'];
 ...
 */
function waterways_preprocess_block(&$vars) {
  $block = $vars['block'];
 ...
 */
function waterways_preprocess_poll_bar(&$vars) {
 ...
 */
function waterways_preprocess_button(&$vars) {
 ...
 */
function waterways_preprocess_menu_local_tasks(&$vars) {
  $vars['primary_widget'] = 'tabs';
  $vars['secondary_widget'] = 'pills';
}

/**
 * Helper function: returns TRUE if current theme is waterways.
 */
function _waterways_is_waterways() {
  global $theme;
  return $theme == 'waterways';
}

/**
  * Helper function: returns markup of all blocks in the region. ss
  */
function _waterways_region_blocks_markup($region) {
...
/**
  ...
  */
function _waterways_verify_jquery_update() {
  if (!module_exists('jquery_update')) {
    _waterways_admin_message(t('<a href="http://drupal.org/project/jquery_update" target="_blank">jQuery Update</a> module is not installed.'), 'error');
    return;
  }
  $version = variable_get('jquery_update_jquery_version');
  if ($version != '1.7') {
    _waterways_admin_message(t('You are now using jQuery @version. Recommended version: jQuery 1.7. <a href="@config">Configure</a>.', array('@version' => $version, '@config' => url('admin/config/development/jquery_update'))), 'warning');
  }
}
...
/**
  ...
  */
function _waterways_detect_twitter_bootstrap() {
  if (module_exists('libraries')) {
    $path = libraries_get_path('bootstrap');
    if (_waterways_check_twitter_bootstrap_path($path)) {
      return $path;
    }
  }
  ...
  _waterways_admin_message(t('<a href="http://getbootstrap.com" target="_blank">Twitter Bootstrap</a> library was not found. Download and extract it here: <code>sites/all/libraries/bootstrap</code>.'), 'error');
  return FALSE;
}
...
/**
  ...
  */
function _waterways_check_twitter_bootstrap_path($path) {
...
/**
  ...
  */
function _waterways_admin_message($message, $type = 'status') {
  if (user_access('administer')) {
    drupal_set_message('Waterways: ' . $message, $type);
  }
}
