...
function getcruiseevents(seldate) {
    ...
    jQuery.ajax({
        ...
        cache: false,
        async: true,
        url: baseUrl + 'get_cruise_events_data_by_selecteddate',
        data: params,
        dataType: "json",
        beforeSend: function() {
            $('#transparentLoader').css('display', 'block');
        ...
        success: function(response, status, req) {
            ...
            if(response=="illegaloperation"){
            	alert("Please Select Valid Date");
            	html = "<option value='No Records'>No Records</option>";
            }
            else if (response.length > 0) {
                for (var i = 0; i < response.length; i++) {
                    html = html + '<option value=' + response[i].id + '>' + response[i].cruise_title +' ('+response[i].port+') '+'-'+response[i].cruisedeparturetime+'</option>';
                }
            }else{
            	html = "<option value='No Records'>No Records</option>";
            }
...
function geteventhtmldata() {
    ...
    jQuery.ajax({
        ...
        cache: false,
        async: true,
        url: baseUrl + 'get_event_details_by_event_id',
        data: params,
        dataType: "json",
        beforeSend: function() {
            $('#transparentLoader').css('display', 'block');
...
function changecruise() {
    ...
    var baseUrl = Drupal.settings.basePath;
    var url = baseUrl + "staffadmin/" + cruisetype + "/" + hiddenorderid + "/" + d1 + "/changecruise";
